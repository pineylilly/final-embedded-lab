...
function HumidityGraph(props: { data?: Array<Array<number>>, min?: string, max?: string }) {
    ...
    const handleCaptureClick = async () => {
        const Elem = document.querySelector<HTMLElement>('#humidity-graph')
        if (!Elem) return
    
        const canvas = await html2canvas(Elem)
        const dataURL = canvas.toDataURL('image/png')
        downloadjs(dataURL, 'humidity-graph.png', 'image/png')
    }

    const handleCSVDownload = async () => {
        export_table_to_csv("humidity", "humidity_csv")
    }
        ...
        <div className="px-1 py-4">
            <div className="flex flex-row pl-12 items-center">
                <div className="pl-3 pr-5 h-max">
                    <h2 className="text-xl align-middle">Humidity</h2>
                </div>
            ...
            <div className="h-96" id="humidity-graph">
                <ResponsiveLine 
                    data={result}
